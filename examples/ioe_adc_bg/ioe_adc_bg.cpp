...
int main() {
  ...
  if(ioe.init()) {
    ...
    ioe.set_mode(PIN_TERM_A, MS51XB9AE::PIN_OUT);
    ioe.set_mode(PIN_TERM_B, MS51XB9AE::PIN_OUT);
    ioe.set_mode(PIN_INPUT, MS51XB9AE::PIN_ADC);
    ...
    while (true) {
      ...
      float analog = ioe.input_as_voltage(PIN_INPUT) / ioe.get_adc_vref();
