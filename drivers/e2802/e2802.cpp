...
namespace pimoroni {
  ...
  void E2802::busy_wait(uint minimum_wait_ms) {
  ...
  void E2802::set_blocking(bool blocking) {
    this->blocking = blocking;
  }

  void E2802::power_off() {
    busy_wait();
    command(POF); // turn off
  }

  void E2802::command(uint8_t reg, size_t len, const uint8_t *data) {
    gpio_put(CS, 0);
  ...
  void E2802::data(size_t len, const uint8_t *data) {
    gpio_put(CS, 0);
    gpio_put(DC, 1); // data mode
    spi_write_blocking(spi, (const uint8_t*)data, len);
    gpio_put(CS, 1);
  ...
  void E2802::command(uint8_t reg, std::initializer_list<uint8_t> values) {
    command(reg, values.size(), (uint8_t *)values.begin());
  }

  void E2802::update(PicoGraphics *graphics) {
