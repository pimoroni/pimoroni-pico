...
namespace pimoroni {
  ...
  bool VL53L1X::init(bool io_2v8) {
    ...
    if(read_reg_16_bit(IDENTIFICATION__MODEL_ID) != 0xEACC) {
      return false;
    }
    ...
    fast_osc_frequency = read_reg_16_bit(OSC_MEASURED__FAST_OSC__FREQUENCY);
    osc_calibrate_val = read_reg_16_bit(RESULT__OSC_CALIBRATE_VAL);
    ...
    write_reg_16_bit(DSS_CONFIG__TARGET_TOTAL_RATE_MCPS, TARGET_RATE); // should already be this value after reset
    ...
    // general config
    write_reg_16_bit(SYSTEM__THRESH_RATE_HIGH, 0x0000);
    write_reg_16_bit(SYSTEM__THRESH_RATE_LOW, 0x0000);
    write_reg(DSS_CONFIG__APERTURE_ATTENUATION, 0x38);

    // timing config
    // most of these settings will be determined later by distance and timing
    // budget configuration
    write_reg_16_bit(RANGE_CONFIG__SIGMA_THRESH, 360); // tuning parm default
    write_reg_16_bit(RANGE_CONFIG__MIN_COUNT_RATE_RTN_LIMIT_MCPS, 192); // tuning parm default
    ...
    write_reg_16_bit(DSS_CONFIG__MANUAL_EFFECTIVE_SPADS_SELECT, 200 << 8);
    ...
    write_reg_16_bit(ALGO__PART_TO_PART_RANGE_OFFSET_MM,
      read_reg_16_bit(MM_CONFIG__OUTER_OFFSET_MM) * 4);
  ...
  uint16_t VL53L1X::get_id() {
    return read_reg_16_bit(IDENTIFICATION__MODEL_ID);
  }

  uint16_t VL53L1X::get_osc() {
    return read_reg_16_bit(OSC_MEASURED__FAST_OSC__FREQUENCY);
  }

  void VL53L1X::set_osc(uint16_t value) {
    write_reg_16_bit(OSC_MEASURED__FAST_OSC__FREQUENCY, value);
  }
  ...
  bool VL53L1X::set_distance_mode_int(uint8_t mode) {
    ...
    switch(mode) {
      case 0:
        ...
        break;
      case 1:
        set_distance_mode(SHORT);
        break;
      case 2:
        set_distance_mode(MEDIUM);
        break;
      case 3:
        set_distance_mode(LONG);
        break;
  ...
  bool VL53L1X::set_measurement_timing_budget(uint32_t budget_us) {
    ...
    macro_period_us = calc_macro_period(read_reg(RANGE_CONFIG__VCSEL_PERIOD_A));
    ...
    write_reg_16_bit(MM_CONFIG__TIMEOUT_MACROP_A, encode_timeout(
      timeout_microseconds_to_mclks(1, macro_period_us)));

    // "Update Range Timing A timeout"
    write_reg_16_bit(RANGE_CONFIG__TIMEOUT_MACROP_A, encode_timeout(
      timeout_microseconds_to_mclks(range_config_timeout_us, macro_period_us)));

    // "Update Macro Period for Range B VCSEL Period"
    macro_period_us = calc_macro_period(read_reg(RANGE_CONFIG__VCSEL_PERIOD_B));

    // "Update MM Timing B timeout"
    // (See earlier comment about MM Timing A timeout.)
    write_reg_16_bit(MM_CONFIG__TIMEOUT_MACROP_B, encode_timeout(
      timeout_microseconds_to_mclks(1, macro_period_us)));

    // "Update Range Timing B timeout"
    write_reg_16_bit(RANGE_CONFIG__TIMEOUT_MACROP_B, encode_timeout(
      timeout_microseconds_to_mclks(range_config_timeout_us, macro_period_us)));
  ...
  uint32_t VL53L1X::get_measurement_timing_budget() {
    ...
    // "Update Macro Period for Range A VCSEL Period"
    uint32_t macro_period_us = calc_macro_period(read_reg(RANGE_CONFIG__VCSEL_PERIOD_A));

    // "Get Range Timing A timeout"

    uint32_t range_config_timeout_us = timeout_mclks_to_microseconds(decode_timeout(
      read_reg_16_bit(RANGE_CONFIG__TIMEOUT_MACROP_A)), macro_period_us);
  ...
  void VL53L1X::start_continuous(uint32_t period_ms) {
    ...
    write_reg_32_bit(SYSTEM__INTERMEASUREMENT_PERIOD, period_ms * osc_calibrate_val);
  ...
  void VL53L1X::stop_continuous() {
    ...
    // "restore vhv configs"
    if(saved_vhv_init != 0) {
      write_reg(VHV_CONFIG__INIT, saved_vhv_init);
    }
    if(saved_vhv_timeout != 0) {
       write_reg(VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, saved_vhv_timeout);
    }
  ...
  void VL53L1X::setup_manual_calibration() {
    // "save original vhv configs"
    saved_vhv_init = read_reg(VHV_CONFIG__INIT);
    saved_vhv_timeout = read_reg(VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND);

    // "disable VHV init"
    write_reg(VHV_CONFIG__INIT, saved_vhv_init & 0x7F);
  ...
  void VL53L1X::update_dss() {
    ...
    if(spadCount != 0) {
      ...
      if(totalRatePerSpad != 0) {
        ...
        // "override DSS config"
        write_reg_16_bit(DSS_CONFIG__MANUAL_EFFECTIVE_SPADS_SELECT, requiredSpads);
    ...
    // "We want to gracefully set a spad target, not just exit with an error"
     ...
     write_reg_16_bit(DSS_CONFIG__MANUAL_EFFECTIVE_SPADS_SELECT, 0x8000);
  }

  // get range, status, rates from results buffer
  // based on VL53L1_GetRangingMeasurementData()
  void VL53L1X::get_ranging_data() {
  ...
  uint32_t VL53L1X::calc_macro_period(uint8_t vcsel_period) {
  ...
  {
    uint8_t buffer[3] = { (uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF), value };
    i2c_write_blocking(i2c, address, buffer, 3, false);
  }

  // Write a 16-bit register
  void VL53L1X::write_reg_16_bit(uint16_t reg, uint16_t value) {
    uint8_t buffer[4] = { (uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF), (uint8_t)((value >> 8) & 0xFF), (uint8_t)(value & 0xFF) };
    i2c_write_blocking(i2c, address, buffer, 4, false);
  }

  // Write a 32-bit register
  void VL53L1X::write_reg_32_bit(uint16_t reg, uint32_t value) {
    uint8_t buffer[6] = { (uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF),
  ...
  uint16_t VL53L1X::read_reg_16_bit(uint16_t reg) {
    uint8_t regbuf[2] = { (uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF) };
    uint8_t buffer[2];
    uint16_t value;
    reg= (reg << 8) + (reg >> 8);
  ...
  uint32_t VL53L1X::read_reg_32_bit(uint16_t reg) {
    uint8_t regbuf[2] = { (uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0x00FF) };
    uint8_t buffer[4];
    uint32_t value;
    reg = (reg << 8) + (reg >> 8);
    i2c_write_blocking(i2c, address, regbuf, 2, true);
