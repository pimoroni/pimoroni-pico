...
namespace pimoroni {
  ...
  public:
   ...
   // PROCESSING_FAIL                 =   8, not used in API
   ...
   // RANGE_VALID_MERGE_PULSE         =  11, not used in API
   ...
   // RANGE_INVALID                   =  14, can't actually be returned by API (range can never become negative, even after correction)
  ...
  public:
    ...
    void write_reg_16_bit(uint16_t reg, uint16_t value);
    void write_reg_32_bit(uint16_t reg, uint32_t value);
    uint8_t read_reg(regAddr reg);
    uint16_t read_reg_16_bit(uint16_t reg);
    uint32_t read_reg_32_bit(uint16_t reg);

    bool set_distance_mode(DistanceMode mode);
    bool set_distance_mode_int(uint8_t mode);
    DistanceMode get_distance_mode() { return distance_mode; }
    ...
    void set_timeout(uint16_t timeout) { io_timeout = timeout; }
    uint16_t get_timeout() { return io_timeout; }
    bool timeout_occurred();
    uint32_t get_curr_ms() { return to_ms_since_boot(get_absolute_time()); }

  private:
      // Record the current time to check an upcoming timeout against
    void start_timeout() { timeout_start_ms = to_ms_since_boot(get_absolute_time()); }

    // Check if timeout is enabled (set to nonzero value) and has expired
    bool check_timeout_expired() {return (io_timeout > 0) && ((to_ms_since_boot(get_absolute_time()) - timeout_start_ms) > io_timeout); }

    void setup_manual_calibration();
    void read_results();
    void update_dss();
    ...
    float count_rate_fixed_to_float(uint16_t count_rate_fixed) { return (float)count_rate_fixed / (1 << 7); }
  };
