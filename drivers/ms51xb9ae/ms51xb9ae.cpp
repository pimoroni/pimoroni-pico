...
namespace pimoroni {
  ...
  const uint8_t MS51XB9AE::Pin::PxM1[4] = {reg::P0M1, reg::P1M1, (uint8_t)-1, reg::P3M1};
  const uint8_t MS51XB9AE::Pin::PxM2[4] = {reg::P0M2, reg::P1M2, (uint8_t)-1, reg::P3M2};
  const uint8_t MS51XB9AE::Pin::Px[4] = {reg::P0, reg::P1, (uint8_t)-1, reg::P3};

  const uint8_t MS51XB9AE::Pin::PxS[4] = {reg::P0S, reg::P1S, (uint8_t)-1, reg::P3S};
  const uint8_t MS51XB9AE::Pin::MASK_P[4] = {reg::INT_MASK_P0, reg::INT_MASK_P1, (uint8_t)-1, reg::INT_MASK_P3};

  const uint8_t MS51XB9AE::Pin::PWML[6] = {reg::PWM0L, reg::PWM1L, reg::PWM2L, reg::PWM3L, reg::PWM4L, reg::PWM5L};
  const uint8_t MS51XB9AE::Pin::PWMH[6] = {reg::PWM0H, reg::PWM1H, reg::PWM2H, reg::PWM3H, reg::PWM4H, reg::PWM5H};
  ...
  MS51XB9AE::Pin::Pin(uint8_t port, uint8_t pin) :
  ...
  MS51XB9AE::Pin::Pin(uint8_t port, uint8_t pin, uint8_t pwm_channel, uint8_t reg_io_pwm) :
  ...
  MS51XB9AE::Pin::Pin(uint8_t port, uint8_t pin, uint8_t adc_channel) :
    type(TYPE_ADC), mode(0), port(port), pin(pin), adc_channel(adc_channel), pwm_channel(0),
    reg_m1(PxM1[port]), reg_m2(PxM2[port]), reg_p(Px[port]), reg_ps(PxS[port]), reg_int_mask_p(MASK_P[port]),
    reg_io_pwm(0), reg_pwml(0), reg_pwmh(0) {
  }

  MS51XB9AE::Pin::Pin(uint8_t port, uint8_t pin, uint8_t adc_channel, uint8_t pwm_channel, uint8_t reg_io_pwm) :
    type(TYPE_ADC_OR_PWM), mode(0), port(port), pin(pin), adc_channel(adc_channel), pwm_channel(pwm_channel),
    reg_m1(PxM1[port]), reg_m2(PxM2[port]), reg_p(Px[port]), reg_ps(PxS[port]), reg_int_mask_p(MASK_P[port]),
    reg_io_pwm(reg_io_pwm), reg_pwml(PWML[pwm_channel]), reg_pwmh(PWMH[pwm_channel]) {
  }

  MS51XB9AE::Pin MS51XB9AE::Pin::io(uint8_t port, uint8_t pin) {
    return Pin(port, pin);
  }

  MS51XB9AE::Pin MS51XB9AE::Pin::pwm(uint8_t port, uint8_t pin, uint8_t channel, uint8_t reg_iopwm) {
    return Pin(port, pin, channel, reg_iopwm);
  }

  MS51XB9AE::Pin MS51XB9AE::Pin::adc(uint8_t port, uint8_t pin, uint8_t channel) {
    return Pin(port, pin, channel);
  }

  MS51XB9AE::Pin MS51XB9AE::Pin::adc_or_pwm(uint8_t port, uint8_t pin, uint8_t adc_channel, uint8_t pwm_channel, uint8_t reg_iopwm) {
    return Pin(port, pin, adc_channel, pwm_channel, reg_iopwm);
  }

  bool MS51XB9AE::Pin::mode_supported(uint8_t mode) {
  ...
  MS51XB9AE::Pin::IOType MS51XB9AE::Pin::get_type() {
    return type;
  }

  uint8_t MS51XB9AE::Pin::get_mode() {
    return mode;
  }

  void MS51XB9AE::Pin::set_mode(uint8_t mode) {
    this->mode = mode;
  }

  MS51XB9AE::MS51XB9AE(i2c_inst_t *i2c, uint8_t sda, uint8_t scl, uint8_t interrupt, uint8_t address, uint32_t timeout, bool debug) :
  ...
  MS51XB9AE::MS51XB9AE(uint8_t address, uint32_t timeout, bool debug) :
    MS51XB9AE(i2c0, DEFAULT_SDA_PIN, DEFAULT_SCL_PIN, DEFAULT_INT_PIN, address, timeout, debug) {
  }
  
  bool MS51XB9AE::init(bool skipChipIdCheck) {
    bool succeeded = true;
  ...
  float MS51XB9AE::get_adc_vref() {
    return vref;
  }

  void MS51XB9AE::set_adc_vref(float vref) {
    this->vref = vref;
  }
    
  void MS51XB9AE::enable_interrupt_out(bool pin_swap) {
    set_bit(reg::INT, int_bit::OUT_EN);
    change_bit(reg::INT, int_bit::PIN_SWAP, pin_swap);
  }

  void MS51XB9AE::disable_interrupt_out() {
    clr_bit(reg::INT, int_bit::OUT_EN);
  }

  uint8_t MS51XB9AE::get_interrupt_flag() {
    if(interrupt != 0)
  ...
  void MS51XB9AE::clear_interrupt_flag() {
    clr_bit(reg::INT, int_bit::TRIGD);
  }

  bool MS51XB9AE::set_pin_interrupt(uint8_t pin, bool enabled) {
  ...
  void MS51XB9AE::set_interrupt_callback(void (*callback)()) {
  ...
  void MS51XB9AE::pwm_load(bool wait_for_load) {
  ...
  bool MS51XB9AE::pwm_loading() {
    return get_bit(reg::PWMCON0, 6);
  }

  void MS51XB9AE::pwm_clear(bool wait_for_clear) {
    uint32_t start_time = millis();
  ...
  bool MS51XB9AE::pwm_clearing() {
    return get_bit(reg::PWMCON0, 4);
  }

  bool MS51XB9AE::set_pwm_control(uint8_t divider) {
  ...
  void MS51XB9AE::set_pwm_period(uint16_t value, bool load) {
    value &= 0xffff;
  ...
  uint8_t MS51XB9AE::get_mode(uint8_t pin) {
    return pins[pin - 1].get_mode();
  }

  void MS51XB9AE::set_mode(uint8_t pin, uint8_t mode, bool schmitt_trigger, bool invert) {
    if(pin < 1 || pin > NUM_PINS)
    {
  ...
  int16_t MS51XB9AE::input(uint8_t pin, uint32_t adc_timeout) {
    if(pin < 1 || pin > NUM_PINS)
    {
      if(debug)
        printf("ValueError: Pin should be in range 1-14.\n");
  ...
  float MS51XB9AE::input_as_voltage(uint8_t pin, uint32_t adc_timeout) {
    if(pin < 1 || pin > NUM_PINS)
    {
      if(debug)
        printf("ValueError: Pin should be in range 1-14.\n");
  ...
  void MS51XB9AE::output(uint8_t pin, uint16_t value, bool load) {
    if(pin < 1 || pin > NUM_PINS) {
  ...
  void MS51XB9AE::setup_rotary_encoder(uint8_t channel, uint8_t pinA, uint8_t pinB, uint8_t pinC, bool count_microsteps) {
    channel -= 1;
  ...
  int16_t MS51XB9AE::read_rotary_encoder(uint8_t channel) {
    channel -= 1;
  ...
  uint8_t MS51XB9AE::i2c_reg_read_uint8(uint8_t reg) {
    uint8_t value;
  ...
  void MS51XB9AE::i2c_reg_write_uint8(uint8_t reg, uint8_t value) {
    uint8_t buffer[2] = {reg, value};
    i2c_write_blocking(i2c, address, buffer, 2, false);
  }

  uint8_t MS51XB9AE::get_bit(uint8_t reg, uint8_t bit) {
    //Returns the specified bit (nth position from right) from a register
    return i2c_reg_read_uint8(reg) & (1 << bit);
  }

  void MS51XB9AE::set_bits(uint8_t reg, uint8_t bits) {
  ...
  void MS51XB9AE::set_bit(uint8_t reg, uint8_t bit) {
    //Set the specified bit (nth position from right) in a register.
    set_bits(reg, (1 << bit));
  }

  void MS51XB9AE::clr_bits(uint8_t reg, uint8_t bits) {
    bool reg_in_bit_addressed_regs = false;
    for(uint8_t i = 0; i < NUM_BIT_ADDRESSED_REGISTERS; i++) {
      if(BIT_ADDRESSED_REGS[i] == reg) {
        for(uint8_t bit = 0; bit < 8; bit++) {
  ...
  void MS51XB9AE::clr_bit(uint8_t reg, uint8_t bit) {
    //Clear the specified bit (nth position from right) in a register.
    clr_bits(reg, (1 << bit));
  }

  void MS51XB9AE::change_bit(uint8_t reg, uint8_t bit, bool state) {
  ...
  void MS51XB9AE::wait_for_flash(void) {
