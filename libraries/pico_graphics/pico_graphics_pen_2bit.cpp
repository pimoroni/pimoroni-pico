...
namespace pimoroni {
    ...
    void PicoGraphics_Pen2Bit::set_pen(uint c) {
        color = c;
    }
    void PicoGraphics_Pen2Bit::set_pen(uint8_t r, uint8_t g, uint8_t b) {
        color = RGB(r, g, b).to_rgb888() | 0x7f000000;
    }
    int PicoGraphics_Pen2Bit::create_pen(uint8_t r, uint8_t g, uint8_t b) {
        return RGB(r, g, b).to_rgb888() | 0x7f000000;
    }
    int PicoGraphics_Pen2Bit::create_pen_hsv(float h, float s, float v) {
        return RGB::from_hsv(h, s, v).to_rgb888() | 0x7f000000;
    }
    void PicoGraphics_Pen2Bit::set_pixel(const Point &p) {
    ...
    void PicoGraphics_Pen2Bit::set_pixel_span(const Point &p, uint l) {
        Point lp = p;
        while(l--) {
            if ((color & 0x7f000000) == 0x7f000000) {
    ...
    void PicoGraphics_Pen2Bit::get_dither_candidates(const RGB &col, const RGB *palette, size_t len, std::array<uint8_t, 16> &candidates) {
        RGB error;
    ...
    void PicoGraphics_Pen2Bit::set_pixel_dither(const Point &p, const RGB &c) {
    ...
    void PicoGraphics_Pen2Bit::frame_convert(PenType type, conversion_callback_func callback) {
        if(type == PEN_P4) {
