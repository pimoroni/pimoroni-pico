...
mp_obj_t BreakoutColourLCD160x80_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    ...
    if(n_args <= 1) {
        ...
        static const mp_arg_t allowed_args[] = {
            ...
            { MP_QSTR_miso, MP_ARG_INT, {.u_int = BreakoutColourLCD160x80::PIN_UNUSED} },
        };
        ...
        if(miso != BreakoutColourLCD160x80::PIN_UNUSED && !IS_VALID_MISO(spi_id, miso)) {
        ...
        self->breakout = new BreakoutColourLCD160x80((uint16_t *)bufinfo.buf, spi, args[ARG_cs].u_int, args[ARG_dc].u_int, sck, mosi, miso);
    }
