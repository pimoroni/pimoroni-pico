...
mp_obj_t BreakoutColourLCD240x240_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    ...
    if(n_args <= 1) {
        ...
        //picodisplay_buf_obj = args[ARG_buffer].u_obj;

        self->breakout = new BreakoutColourLCD240x240((uint16_t *)bufinfo.buf);     
    }
    else {
        enum { ARG_buffer, ARG_spi, ARG_cs, ARG_dc, ARG_sck, ARG_mosi, ARG_miso };
        static const mp_arg_t allowed_args[] = {
            { MP_QSTR_buffer, MP_ARG_REQUIRED | MP_ARG_OBJ },
            ...
            { MP_QSTR_miso, MP_ARG_INT, {.u_int = BreakoutColourLCD240x240::PIN_UNUSED} },
        };
        ...
        if(miso != BreakoutColourLCD240x240::PIN_UNUSED && !IS_VALID_MISO(spi_id, miso)) {
        ...
        self->breakout = new BreakoutColourLCD240x240((uint16_t *)bufinfo.buf, spi, args[ARG_cs].u_int, args[ARG_dc].u_int, sck, mosi, miso);
    }
